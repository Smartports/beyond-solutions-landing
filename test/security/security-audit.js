...
function generateRecommendations(results) {
  ...
  for (const [pageName, pageData] of Object.entries(results.pages)) {
    if (pageData.tests['Content Security Policy'] && pageData.tests['Content Security Policy'].status === 'failed') {
      recommendations.critical.push(`Implement proper Content Security Policy for ${pageName}.`);
    } else if (pageData.tests['Content Security Policy'] && pageData.tests['Content Security Policy'].status === 'warning') {
      recommendations.important.push(`Strengthen Content Security Policy for ${pageName}.`);
    }
    
    // Check for XSS vulnerabilities
    if (pageData.tests['XSS Vulnerabilities'] && pageData.tests['XSS Vulnerabilities'].status === 'failed') {
      recommendations.critical.push(`Fix XSS vulnerabilities in ${pageName}.`);
    }
    
    // Check for local storage security
    if (pageData.tests['Local Storage Security'] && pageData.tests['Local Storage Security'].status === 'failed') {
      recommendations.important.push(`Improve local storage security in ${pageName}.`);
    }
    
    // Check for input validation
    if (pageData.tests['Input Validation'] && pageData.tests['Input Validation'].status === 'failed') {
      recommendations.important.push(`Implement proper input validation in ${pageName}.`);
    }
    
    // Check for API access controls
    if (pageData.tests['API Access Controls'] && pageData.tests['API Access Controls'].status === 'failed') {
      recommendations.critical.push(`Fix API access control issues in ${pageName}.`);
    } else if (pageData.tests['API Access Controls'] && pageData.tests['API Access Controls'].status === 'warning') {
  ...
  for (let i = 0; i < inputFields.length; i++) {
    ...
    for (const payload of xssPayloads) {
      ...
      if (bodyHTML.includes(payload)) {
        vulnerabilities.push(`Potential XSS in ${inputName}: payload "${payload}" was reflected unescaped`);
      }
    ...
    for (const input of applicableInputs) {
      ...
      if (form.hasForm) {
        ...
        if (url.includes('?') || url.includes('#')) {
          if (!form.isValid) {
            validationIssues.push(`Form submitted with invalid input "${input.name}" in field "${inputName}"`);
          }
        }
      } else {
        ...
        if (!hasError && !form.isValid) {
          validationIssues.push(`No validation error for invalid input "${input.name}" in field "${inputName}"`);
        }
      }
    }
  }
...
async function testAPIAccess(page) {
  ...
  for (const call of apiCalls) {
    ...
    if (!hasAuth && (call.method !== 'GET' || call.url.includes('user') || call.url.includes('admin'))) {
