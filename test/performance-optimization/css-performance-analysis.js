...
const cssFiles = ['css/colors.css', 'css/rtl.css', 'css/language-selector.css', 'i18n/rtl.css'];
  ...
  cssFiles.forEach((file) => {
    try {
      ...
      const colorProperties = (
        content.match(/(?:color|background|border|box-shadow):[^;]+;/g) || []
      ).length;
...
function generateReport(staticAnalysis, dynamicAnalysis) {
  ...
  if (avgUsage < 50) {
    ...
    report +=
      '⚠️ **Moderate amount of unused CSS detected.** Consider reviewing and optimizing CSS usage.\n\n';
  } else {
  ...
  if (totalCssVars > 50) {
    report +=
      '⚠️ **High number of CSS variables.** While variables improve maintainability, too many can impact performance. Consider consolidating similar variables.\n\n';
  } else {
    report +=
  ...
  if (totalFileSize > 50000) {
    report +=
      '⚠️ **CSS files are quite large.** Consider minification and removing unused styles.\n\n';
  } else {
    report +=
