...
async function testColorContrast(page) {
  ...
  const elements = await page.$$('h1, h2, h3, h4, h5, h6, p, span, a, button, label, input, select, textarea');
  const results = {
    tested: 0,
    passed: 0,
    failed: 0,
  ...
  for (const element of elements) {
    try {
      ...
      const isLargeText = styles.fontSize >= 24 || (styles.fontSize >= 18.66 && parseInt(styles.fontWeight) >= 700);
      ...
      if (passes) {
        ...
        results.issues.push({
          ...
          mode: await page.evaluate(() => window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light')
        });
      }
    } catch (error) {
...
function generateAccessibilityReport(results) {
  ...
  if (results.summary.totalViolations === 0) {
    ...
    if (results.summary.colorContrastViolations > 0) {
      ...
      report += '- Review and adjust the color palette to ensure all text meets WCAG 2.1 AA contrast requirements\n';
