...
async function testPagePerformance(browser, url, mode) {
  ...
  try {
    ...
    const performanceMetrics = await page.evaluate(() => {
      ...
      return {
        domContentLoaded: perfEntries.length > 0 ? perfEntries[0].domContentLoadedEventEnd - perfEntries[0].domContentLoadedEventStart : null,
    ...
    const renderingMetrics = await page.evaluate(() => {
      ...
      return {
        ...
        layoutTime: performance.getEntriesByType('layout-shift').reduce((sum, entry) => sum + entry.value, 0)
      };
    });
    ...
    const cssInfo = await page.evaluate(() => {
      ...
      const colorRules = cssRules.filter(rule => 
        rule.cssText && (
...
function generateSummary(results) {
  ...
  if (averages.dark.navigationTime > averages.light.navigationTime * 1.2) {
    summary += '⚠️ **Dark mode is significantly slower than light mode.** Consider optimizing dark mode styles.\n\n';
  } else {
    summary += '✅ **Dark mode performance is comparable to light mode.** The color palette implementation is efficient.\n\n';
  }
  
  if (averages.light.firstContentfulPaint > 1000 || averages.dark.firstContentfulPaint > 1000) {
    summary += '⚠️ **First contentful paint is slow.** Consider optimizing critical CSS or reducing render-blocking resources.\n\n';
  } else {
    summary += '✅ **First contentful paint is fast.** The color palette does not negatively impact initial rendering.\n\n';
  }
