...
function isInPalette(color) {
  ...
  return Object.values(colorPalette).some(paletteColor => 
    normalizeColor(paletteColor) === normalizedColor
  );
}
  ...
  async function scan(directory) {
    ...
    for (const entry of entries) {
      ...
      if (stats.isDirectory()) {
        ...
        if (filePatterns.html.test(entry) || 
...
function generateReport() {
  ...
  if (results.hardcodedColors.length > 0) {
    report += '\n## Hardcoded Colors\n\n';
    report += 'These are colors from our palette that are hardcoded instead of using CSS variables.\n\n';
  ...
  if (results.hardcodedColors.length > 0) {
    ...
    report += 'Replace all hardcoded color values with CSS variables to ensure consistency and easier updates in the future.\n\n';
    report += 'Example:\n\n';
    report += '```css\n';
    report += '/* Before */\n';
    report += 'color: #334b4e;\n\n';
  ...
  if (results.missingVariables.length > 0) {
    ...
    report += 'Either define the missing variables or update the references to use variables from the official color palette.\n\n';
  }
  
  if (results.deprecatedColors.length > 0) {
    report += '### Update Deprecated Color References\n\n';
    report += 'Replace all references to the old color system with the new color palette variables.\n\n';
    report += 'Example:\n\n';
    report += '```css\n';
    report += '/* Before */\n';
...
async function main() {
  ...
  try {
    ...
    const reportPath = path.join(outputDir, `color-consistency-report-${new Date().toISOString().replace(/:/g, '-')}.md`);
    ...
    // Output summary
    console.log('\nSummary:');
    console.log(`- Files with issues: ${results.fileStats.filesWithIssues}/${results.fileStats.totalFiles}`);
