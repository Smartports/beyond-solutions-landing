...
function debugI18nState() {
  ...
  try {
    ...
    const currentLanguage =
      window.i18n?.getCurrentLanguage?.() || localStorage.getItem('beyondLocale') || 'unknown';
    ...
    if (tFunctionAvailable) {
      ...
      try {
        ...
        console.log(
          `✅ Translation result: "${translationResult}" (success: ${translationResult !== testKey})`,
        );
      } catch (e) {
...
window.preloadAllTranslations = async function preloadAllTranslations() {
  try {
    ...
    const sortedLanguages = [currentLang, ...languages.filter((lang) => lang !== currentLang)];
...
document.addEventListener('alpine:init', () => {
  ...
  Alpine.data('calculatorForm', () => ({
    ...
    translateElementsInContainer(container, t) {
      ...
      container.querySelectorAll('[data-i18n]').forEach((el) => {
        ...
        if (key && el.tagName !== 'OPTION') {
          // Skip options as they're handled above
      ...
      container.querySelectorAll('[data-i18n-attr]').forEach((el) => {
        ...
        if (attrSpec) {
          attrSpec.split(',').forEach((spec) => {
            ...
            if (attr && key && attr !== 'placeholder') {
              // Skip placeholders as they're handled above
    ...
    errors: {},
    tooltipTexts: {
      budgetTotal: safeTranslate(
        'calculator.tooltips.budgetTotal',
        'Incluye terreno y construcción, sin regulatorio.',
      ),
      usableSurface: safeTranslate(
        'calculator.tooltips.usableSurface',
        'Superficie útil construible según uso de suelo.',
      ),
      materialesLevel: safeTranslate(
        'calculator.tooltips.materialesLevel',
        'Define el rango de calidad y costo de materiales.',
      ),
    },
    feedbackMessage: '',
    feedbackType: '',
    ...
    validateStep() {
      ...
      if (this.currentStep === 0) {
        if (!this.form.scope) {
          this.errors.scope = safeTranslate(
            'calculator.errors.scope',
            'El tipo de proyecto es obligatorio',
          );
        }
        if (!this.form.entity) {
          this.errors.entity = safeTranslate(
            'calculator.errors.entity',
      ...
      if (this.currentStep === 1) {
        if (!this.form.budgetTotal || this.form.budgetTotal <= 0) {
          this.errors.budgetTotal = safeTranslate(
            'calculator.errors.budgetTotal',
            'El presupuesto debe ser mayor a 0',
          );
        }
        if (!this.form.land.address) {
          this.errors.address = safeTranslate(
            'calculator.errors.address',
            'La dirección es obligatoria',
          );
        }
        if (!this.form.land.type) {
          this.errors.type = safeTranslate(
            'calculator.errors.type',
            'El tipo de terreno es obligatorio',
          );
        }
        if (!this.form.land.status) {
          this.errors.status = safeTranslate(
            'calculator.errors.status',
            'El estatus es obligatorio',
          );
        }
        if (!this.form.land.surface || this.form.land.surface <= 0) {
          this.errors.surface = safeTranslate(
            'calculator.errors.surface',
            'La superficie debe ser mayor a 0',
          );
        }
        if (!this.form.land.usableSurface || this.form.land.usableSurface <= 0) {
          this.errors.usableSurface = safeTranslate(
            'calculator.errors.usableSurface',
            'La superficie útil debe ser mayor a 0',
          );
        }
      }
      ...
      if (this.currentStep === 2) {
        for (const key of this.costKeys) {
          if (this.form.costs[key].percent === null || this.form.costs[key].percent < 0) {
            this.errors[key] = safeTranslate(
              'calculator.errors.percent',
    ...
    goToStep(stepNumber) {
      ...
      if (Math.abs(leavingStep - stepNumber) > 0) {
        console.log(
          `Special handling for transition from step ${leavingStep + 1} to step ${stepNumber + 1}`,
        );
    ...
    exportPDF() {
      try {
        ...
        let y = 30;
        doc.text(
          `${t('calculator.summary.scope', 'Tipo de proyecto')}: ${this.form.scope ? t(`calculator.values.scope.${this.form.scope}`, this.form.scope) : ''}`,
          10,
          y,
        );
        y += 8;
        doc.text(
          `${t('calculator.summary.entity', 'Entidad')}: ${this.form.entity ? t(`calculator.values.entity.${this.form.entity}`, this.form.entity) : ''}`,
          10,
          y,
        );
        y += 8;
        ...
        for (const key of this.costKeys) {
          doc.text(
            `${this.costLabels[key]}: ${(this.form.costs[key].value || 0).toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })} (${((this.form.costs[key].percent || 0) * 100).toFixed(2)}%)`,
            10,
            y,
          );
          y += 8;
        ...
        this.showFeedback(
          safeTranslate('calculator.feedback.pdfSuccess', 'PDF generado correctamente.'),
          'success',
        );
      } catch (e) {
        this.showFeedback(
          safeTranslate('calculator.feedback.pdfError', 'Error al exportar PDF.'),
          'error',
        );
      }
    },
    exportCSV() {
      try {
        ...
        const data = [
          ...
          ...this.costKeys.map((key) => [
            this.costLabels[key],
            this.form.costs[key].value || 0,
            ((this.form.costs[key].percent || 0) * 100).toFixed(2) + '%',
          ]),
          [],
        ...
        this.showFeedback(
          safeTranslate('calculator.feedback.csvSuccess', 'CSV generado correctamente.'),
          'success',
        );
      } catch (e) {
        this.showFeedback(
          safeTranslate('calculator.feedback.csvError', 'Error al exportar CSV.'),
          'error',
        );
      }
    },
    shareWA() {
      try {
        ...
        this.showFeedback(
          safeTranslate('calculator.feedback.waSuccess', 'Compartido en WhatsApp.'),
          'success',
        );
      } catch (e) {
        this.showFeedback(
          safeTranslate('calculator.feedback.waError', 'Error al compartir en WhatsApp.'),
          'error',
        );
      }
    },
    shareEmail() {
      try {
        ...
        this.showFeedback(
          safeTranslate('calculator.feedback.emailSuccess', 'Correo abierto.'),
          'success',
        );
      } catch (e) {
        this.showFeedback(
          safeTranslate('calculator.feedback.emailError', 'Error al compartir por Email.'),
          'error',
        );
      }
    },
    saveSimulation() {
      try {
        ...
        this.showFeedback(
          safeTranslate('calculator.feedback.saved', 'Simulación guardada.'),
          'success',
        );
      } catch (e) {
        this.showFeedback(
          safeTranslate('calculator.feedback.saveError', 'Error al guardar.'),
          'error',
        );
      }
    },
    restoreSimulation() {
      try {
        ...
        if (form && results) {
          ...
          this.currentStep = 3;
          this.showFeedback(
            safeTranslate('calculator.feedback.restored', 'Simulación restaurada.'),
            'success',
          );
        } else {
          this.showFeedback(
            safeTranslate('calculator.feedback.noSaved', 'No hay simulación guardada.'),
            'info',
          );
        }
      } catch (e) {
        this.showFeedback(
          safeTranslate('calculator.feedback.restoreError', 'Error al restaurar.'),
          'error',
        );
      }
    },
    ...
    $watch: {
      'form.budgetTotal': function () {
        this.calculateResults();
      },
      'form.land.usableSurface': function () {
        this.calculateResults();
      },
      'form.costs': {
        deep: true,
    ...
    fixSpecificElements() {
      ...
      try {
        ...
        const problematicElements = [
          ...
          {
            selector: '#step1 select[name="scope"] option:first-child',
            key: 'calculator.step1.scope.select',
          },
          {
            selector: '#step1 select[name="entity"] option:first-child',
            key: 'calculator.step1.entity.select',
          },
          ...
          {
            selector: '#step2 .form-group:nth-child(6) label',
            key: 'calculator.step2.usableSurface',
          },
          { selector: '#step2 .form-group:nth-child(7) label', key: 'calculator.step2.use' },
          {
            selector: '#step2 .form-group:nth-child(8) label',
            key: 'calculator.step2.characteristics',
          },
          {
            selector: '#step2 select[name="type"] option:first-child',
            key: 'calculator.step2.type.select',
          },
          {
            selector: '#step2 select[name="status"] option:first-child',
            key: 'calculator.step2.status.select',
          },
        ];
            ...
            elements.forEach((element) => {
              ...
              if (translation === item.key) {
                ...
                if (lastPart === 'scope') {
                  ...
                  translation = window.i18n?.t(
                    'calculator.form.usableSurface',
                    {},
                    'Usable surface',
                  );
                  ...
                  translation = window.i18n?.t(
                    'calculator.form.characteristics',
                    {},
                    'Characteristics',
                  );
                ...
                if (
                  ...
                  translation =
                    lastPart.charAt(0).toUpperCase() + lastPart.slice(1).replace(/([A-Z])/g, ' $1');
                }
              }
              ...
              if (translation && !translation.includes('[') && !translation.includes(']')) {
                console.log(
                  `Fixing element "${item.selector}" with key "${item.key}" to "${translation}"`,
                );
        ...
        const placeholderElements = [
          ...
          {
            selector: '#step2 input[name="usableSurface"]',
            key: 'calculator.step2.usableSurface.placeholder',
          },
          { selector: '#step2 input[name="use"]', key: 'calculator.step2.use.placeholder' },
          {
            selector: '#step2 textarea[name="characteristics"]',
            key: 'calculator.step2.characteristics.placeholder',
          },
        ];
        ...
        placeholderElements.forEach((item) => {
          ...
          if (element) {
            ...
            if (translation && translation !== item.key) {
              ...
              console.log(
                `Setting fallback placeholder for "${item.selector}" to "${placeholder}"`,
              );
              ...
              if (key) {
                ...
                if (forAttr) {
                  ...
                  if (translation !== inferredKey) {
                    console.log(
                      `Updating hidden label with inferred key "${inferredKey}" to "${translation}"`,
                    );
                    label.textContent = translation;
                  }
                }
            ...
            step.querySelectorAll('select').forEach((select) => {
              ...
              if (name) {
                ...
                if (firstOption) {
                  ...
                  if (translation !== inferredKey) {
                    console.log(
                      `Updating hidden select option with inferred key "${inferredKey}" to "${translation}"`,
                    );
