...
export async function initI18n(customOptions = {}) {
  ...
  if (!detectedLanguage) {
    console.error(
      '[i18n] CRITICAL: detectLanguage() returned null/undefined language. Defaulting to:',
  ...
  console.log(
    `[i18n] Initializing with detected language: ${languageToUse} (detected: ${detectedLanguage || 'NONE'})`,
  );
  ...
  if (!currentLanguage) {
    console.error(
      '[i18n] CRITICAL: After initialization, currentLanguage is still null! Forcing default language.',
    );
...
function translateNode(node) {
  ...
  const elements =
    node.nodeType === 1 && node.matches('[data-i18n], [data-i18n-attr]')
...
async function loadTranslations(langCode) {
  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to load translations from alternative path: ${response.status} ${response.statusText}`,
        );
      }
      ...
      console.error(
        `[i18n] Error loading translations from alternative path for ${langCode}:`,
        altError,
      );
...
function fixCalculatorTranslationKeys(translations) {
  if (!translations || typeof translations !== 'object') {
    console.warn(
      '[i18n] Cannot fix calculator keys: translations is not an object',
      typeof translations,
    );
    return;
  }
  ...
  try {
    ...
    if (
      !translations.calculator.summary.usableSurface &&
      translations.calculator.step2.usableSurface
    ) {
...
export function t(key, params = {}, defaultValue = '') {
  ...
  const shouldLogWarnings = typeof window !== 'undefined' && window.i18nFullyInitialized === true;
  ...
  if (!translationsCache.has(currentLanguage)) {
    ...
    if (shouldLogWarnings) {
      console.warn(
        `[i18n] No translations loaded for "${currentLanguage}" (language code: ${typeof currentLanguage}), returning default`,
        {
  ...
  try {
    ...
    for (const k of keys) {
      if (value && typeof value === 'object' && k in value) {
        ...
        if (options.debug) {
          console.warn(
            `[i18n] Key part "${k}" not found in path "${key}" for language "${currentLanguage}"`,
          );
        }
        ...
        if (
          currentLanguage !== options.fallbackLanguage &&
          translationsCache.has(options.fallbackLanguage)
        ) {
          if (options.debug) {
            console.log(
              `[i18n] Trying fallback language "${options.fallbackLanguage}" for key "${key}"`,
            );
          }
          ...
          for (const k of keys) {
            if (fallbackValue && typeof fallbackValue === 'object' && k in fallbackValue) {
              ...
              if (options.debug) {
                console.warn(
                  `[i18n] Key "${key}" not found in fallback language "${options.fallbackLanguage}" either`,
                );
              }
              return defaultValue || key;
            }
          ...
          if (typeof fallbackValue === 'string') {
            ...
            if (shouldLogWarnings && options.debug) {
              console.warn(
                `[i18n] Value for key "${key}" in fallback language is not a string: ${typeof fallbackValue}`,
              );
            }
            return defaultValue || key;
          }
          ...
          if (shouldLogWarnings && options.debug) {
            console.warn(
              `[i18n] Key "${key}" not found in current language and no fallback available`,
            );
          }
          return defaultValue || key;
        }
...
function getCalculatorFallback(key, params, defaultValue) {
  ...
  if (lastPart) {
    ...
    const fallback = lastPart.charAt(0).toUpperCase() + lastPart.slice(1).replace(/[_-]/g, ' ');
...
export async function ensureAllLanguagesSupport() {
  ...
  try {
    ...
    console.log(
      `[i18n] Preloading translations for ${supportedLanguages.length} languages: ${supportedLanguages.join(', ')}`,
    );
    ...
    console.log(
      `[i18n] Preloaded translations for ${loadedLanguages.length}/${supportedLanguages.length} languages:`,
      loadedLanguages,
    );
