...
(function() {
  ...
  window.FinanceModule = function() {
    ...
    const state = {
      ...
      financialModel: {
        ...
        additionalCosts: {
          ...
          utilities: 0.10,         // Instalaciones y conexiones
          landscaping: 0.05,       // Paisajismo
          contingency: 0.10,       // Contingencia
        ...
        marketParams: {
          ...
          vacancyRate: 0.10,       // Tasa de vacancia
        ...
        scenarios: {
          ...
          pessimistic: {
            ...
            constructionTimeMultiplier: 1.20,
    ...
    function calculateRevenue() {
      ...
      const monthlyRentPerM2 = salePricePerM2 * state.financialModel.marketParams.rentalYield / 12;
      const monthlyRentRevenue = area * monthlyRentPerM2 * (1 - state.financialModel.marketParams.vacancyRate);
    ...
    function calculateCashFlows() {
      ...
      const constructionCostPerYear = (costs.constructionCost + costs.totalAdditional) / constructionYears;
      ...
      for (let year = constructionYears + 1; year <= years; year++) {
        ...
        // Aplicar apreciación
        const appreciation = Math.pow(1 + state.financialModel.marketParams.priceAppreciation, yearsSinceCompletion);
      ...
      const residualValue = revenue.totalSalesRevenue * Math.pow(1 + state.financialModel.marketParams.priceAppreciation, years);
    ...
    function calculateIndicators() {
      ...
      const totalInvestment = Math.abs(cashFlows.filter(cf => cf.netCashFlow < 0)
                                              .reduce((sum, cf) => sum + cf.netCashFlow, 0));
      const totalReturn = cashFlows.filter(cf => cf.netCashFlow > 0)
                                   .reduce((sum, cf) => sum + cf.netCashFlow, 0);
      ...
      for (let i = 0; i < cashFlows.length; i++) {
        ...
        if (cumulativeCashFlow >= 0) {
          paybackPeriod = i + (cumulativeCashFlow - cashFlows[i].netCashFlow) / Math.abs(cashFlows[i].netCashFlow);
          break;
        }
      }
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: (context) => {
                  const percentage = costs.breakdown[context.label.toLowerCase().replace(' adicionales', '')];
    ...
    async function exportToPDF() {
      ...
      doc.text(`Período de Recuperación: ${state.calculations.indicators.paybackPeriod} años`, 20, 130);
      
      // Resumen de costos
      doc.setFontSize(14);
      doc.text('Resumen de Costos', 20, 150);
      doc.setFontSize(11);
      doc.text(`Costo Total: $${(state.calculations.costs.totalCost / 1000000).toFixed(2)}M MXN`, 20, 160);
      doc.text(`Costo por m²: $${state.calculations.costs.costPerM2.toLocaleString()} MXN`, 20, 170);
    ...
    function exportToExcel() {
      ...
      csvData.push(['Año', 'Inversión', 'Construcción', 'Ingresos', 'Gastos Operativos', 'Flujo Neto']);
    ...
    function getSummary() {
      ...
      return {
        ...
        viability: {
          isViable: state.calculations.indicators.npv > 0 && parseFloat(state.calculations.indicators.irr) > 12,
