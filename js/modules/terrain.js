  ...
  return {
    ...
    async loadAdditionalLibraries() {
      return new Promise((resolve, reject) => {
        ...
        const apiKey =
          window.BEYOND_CONFIG?.GOOGLE_MAPS_API_KEY || 'AIzaSyCHZEIwEo3h7Ah3skmMbyMOEcj6H85eG_c';
      ...
      this.geofenceConfig.allowedZones = [
        {
          ...
          coordinates: [
            { lat: 19.44, lng: -99.205 },
            { lat: 19.445, lng: -99.2 },
            { lat: 19.44, lng: -99.195 },
            { lat: 19.435, lng: -99.2 },
          ],
        },
        {
          ...
          coordinates: [
            { lat: 19.365, lng: -99.26 },
            { lat: 19.37, lng: -99.255 },
            { lat: 19.365, lng: -99.25 },
            { lat: 19.36, lng: -99.255 },
          ],
        },
      ];
      ...
      this.geofenceConfig.restrictedZones = [
        {
          ...
          coordinates: [
            { lat: 19.26, lng: -99.105 },
            { lat: 19.265, lng: -99.1 },
            { lat: 19.26, lng: -99.095 },
            { lat: 19.255, lng: -99.1 },
          ],
        },
      ];
    ...
    clearGeofencing() {
      ...
      [...this.geofenceConfig.allowedZones, ...this.geofenceConfig.restrictedZones].forEach(
        (zone) => {
          if (zone.polygon) {
      ...
      path.forEach((vertex) => {
        if (
          !google.maps.geometry.poly.containsLocation(
            vertex,
            new google.maps.Polygon({ paths: zoneCoords }),
          )
        ) {
          allInside = false;
        }
    ...
    async getElevationProfile() {
      ...
      const path = this.terrainData.coordinates.map(
    ...
    calculateSlope() {
      if (!this.terrainData.elevationProfile || this.terrainData.elevationProfile.length < 2)
        return;
      ...
      // Average slope
      this.terrainData.slope =
        totalDistance > 0 ? ((totalElevationChange / totalDistance) * 100).toFixed(2) : 0;
    ...
    toggleMeasureTool() {
      ...
      if (!this.measureMode) {
        ...
        const measurePolylineListener = google.maps.event.addListener(
          this.drawingManager,
          'polylinecomplete',
          (polyline) => {
            // Calcular longitud
    ...
    updateTerrainInfo() {
      ...
      window.dispatchEvent(
        new CustomEvent('terrainUpdated', {
