...
export function calculateBudget(
  ...
  const indirectCosts: Record<IndirectCostType, IndirectCost> = {} as Record<
    IndirectCostType,
    IndirectCost
  >;
  ...
  Object.values(IndirectCostType).forEach((costType) => {
    ...
    const percentage =
      indirectCostPercentages[costType] !== undefined
...
export function calculateDetailedBreakdown(budget: Budget): Record<string, number> {
  ...
  breakdown['Cerramientos'] = constructionCost * 0.2;
  breakdown['Cubiertas'] = constructionCost * 0.1;
  breakdown['Instalaciones MEP'] = constructionCost * 0.3;

  // Desglosar materiales
  const materialsCost = budget.directCosts[DirectCostType.MATERIALS].amount;
  breakdown['Materiales estructurales'] = materialsCost * 0.3;
  breakdown['Acabados'] = materialsCost * 0.4;
  breakdown['Equipamiento'] = materialsCost * 0.3;
