...
export function createPredefinedScenario(
  ...
  const defaults: Record<ScenarioType, Partial<ScenarioConfig>> = {
    [ScenarioType.OPTIMISTIC]: {
      ...
      salesConfig: {
        salesVelocity: baseConfig.salesConfig?.totalUnits
          ? baseConfig.salesConfig.totalUnits / 12
          : 5,
    ...
    [ScenarioType.REALISTIC]: {
      ...
      salesConfig: {
        salesVelocity: baseConfig.salesConfig?.totalUnits
          ? baseConfig.salesConfig.totalUnits / 18
          : 3,
    ...
    [ScenarioType.PESSIMISTIC]: {
      ...
      salesConfig: {
        salesVelocity: baseConfig.salesConfig?.totalUnits
          ? baseConfig.salesConfig.totalUnits / 24
          : 2,
  ...
  const config: ScenarioConfig = {
    ...
    salesConfig: {
      ...
      salesVelocity:
        defaults[type]?.salesConfig?.salesVelocity || baseConfig.salesConfig?.salesVelocity || 3,
      priceIncreaseRate:
        defaults[type]?.salesConfig?.priceIncreaseRate ||
        baseConfig.salesConfig?.priceIncreaseRate ||
        3.0,
      ...
      projectDuration:
        defaults[type]?.salesConfig?.projectDuration ||
        baseConfig.salesConfig?.projectDuration ||
        30,
    },
    financingConfig: {
      type: baseConfig.financingConfig?.type || FinancingType.TRADITIONAL_MORTGAGE,
      downPaymentPercentage:
        defaults[type]?.financingConfig?.downPaymentPercentage ||
        baseConfig.financingConfig?.downPaymentPercentage ||
        20,
      interestRate:
        defaults[type]?.financingConfig?.interestRate ||
        baseConfig.financingConfig?.interestRate ||
        8.5,
...
function findBestScenario(
  ...
  for (let i = 1; i < scenarios.length; i++) {
    ...
    if (
      (criteria === 'max' && currentValue > bestValue) ||
...
export function generateFiveYearProjection(scenario: Scenario): Record<string, any> {
  ...
  projection.cumulativeValues.roi =
    projection.cumulativeValues.costs > 0
