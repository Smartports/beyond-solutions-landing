...
export const ConstructionSystemSelector: React.FC<ConstructionSystemSelectorProps> = ({
  ...
  const [system, setSystem] = useState<ConstructionSystem>(
    initialSystem || {
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          >
            {Object.values(StructureType).map((type) => (
              <option key={type} value={type}>
                {getStructureTypeName(type)} (${CONSTRUCTION_SYSTEM_COSTS.structure[type].costPerM2}
                /m²)
              </option>
            ))}
          </select>
        ...
        <div>
          ...
          >
            {Object.values(EnclosureType).map((type) => (
              <option key={type} value={type}>
                {getEnclosureTypeName(type)} (${CONSTRUCTION_SYSTEM_COSTS.enclosure[type].costPerM2}
                /m²)
              </option>
            ))}
          </select>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Tipo de Cubierta</label>
          <select
            className="w-full p-2 border border-gray-300 rounded"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Sistema MEP</label>
          <select
            className="w-full p-2 border border-gray-300 rounded"
          ...
          >
            {Object.values(MepSystemType).map((type) => (
              <option key={type} value={type}>
                {getMepSystemTypeName(type)} (${CONSTRUCTION_SYSTEM_COSTS.mepSystem[type].costPerM2}
                /m²)
              </option>
            ))}
          </select>
