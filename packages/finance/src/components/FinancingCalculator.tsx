...
export const FinancingCalculator: React.FC<FinancingCalculatorProps> = ({
  ...
  const [interestRate, setInterestRate] = useState<number>(
    initialScheme?.interestRate || 8.5
  );
  const [term, setTerm] = useState<number>(
    initialScheme?.term || 20
  );
  ...
  const [activeTab, setActiveTab] = useState<'summary' | 'amortization' | 'taxes' | 'comparison'>('summary');
  ...
  const createAlternativeSchemes = useCallback((): FinancingScheme[] => {
    ...
    if (term > 10) {
      schemes.push(createFinancingScheme(
        financingType,
        totalAmount,
    ...
    const alternativeType = financingType === FinancingType.TRADITIONAL_MORTGAGE
          ...
          <div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <p className="text-xs text-gray-500">{100 - scheme.downPaymentPercentage}% del total</p>
              </div>
              <div className="bg-purple-50 p-4 rounded-lg">
                <p className="text-sm text-gray-500">Pago {getPaymentFrequencyName(paymentFrequency)}</p>
                <p className="text-2xl font-bold">
                  ${amortizationTable.length > 0 ? amortizationTable[0].payment.toLocaleString(undefined, { maximumFractionDigits: 2 }) : 0}
                </p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <p className="text-lg font-semibold">
                    ${amortizationTable.reduce((sum, entry) => sum + entry.interest, 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Comisión por Apertura</p>
                  <p className="text-lg font-semibold">${scheme.originationFeeAmount.toLocaleString()}</p>
                </div>
                <div>
          ...
          <div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {Object.entries(comparisonResult.schemes).map(([schemeId, data]: [string, any]) => (
                    <tr key={schemeId} className={schemeId === scheme.type ? 'bg-blue-50' : ''}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        {data.name} {schemeId === scheme.type && '(Actual)'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-500">
                        ${data.monthlyPayment.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-500">
                        ${data.totalInterest.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-500">
            ...
            <div className="mt-6 bg-gray-50 p-4 rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-lg font-semibold">
                    {comparisonResult.bestOption.lowestMonthlyPayment.schemeId === scheme.type ? 'Esquema Actual' : getFinancingTypeName(comparisonResult.bestOption.lowestMonthlyPayment.schemeId as FinancingType)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Menor Interés Total</p>
                  <p className="text-lg font-semibold">
                    {comparisonResult.bestOption.lowestTotalInterest.schemeId === scheme.type ? 'Esquema Actual' : getFinancingTypeName(comparisonResult.bestOption.lowestTotalInterest.schemeId as FinancingType)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Menor Costo Total</p>
                  <p className="text-lg font-semibold">
                    {comparisonResult.bestOption.lowestTotalCost.schemeId === scheme.type ? 'Esquema Actual' : getFinancingTypeName(comparisonResult.bestOption.lowestTotalCost.schemeId as FinancingType)}
                  </p>
                </div>
              </div>
            </div>
