import { Scene, Vector3, Animation, AbstractMesh, Color3, MeshBuilder, StandardMaterial } from '@babylonjs/core';
...
export class ViewManager {
  ...
  constructor(viewer: ImmersiveViewer, private options: ViewManagerOptions = {}) {
    this.viewer = viewer;
  ...
  private createViewLabel(viewPoint: ViewPoint): void {
    ...
    material.diffuseColor = viewPoint.type === ViewType.EXTERIOR 
  ...
  private createMinimap(): void {
    ...
    this.minimapMesh = MeshBuilder.CreatePlane('minimap', { width: size, height: size }, this.scene);
    ...
    this.minimapMarker = MeshBuilder.CreateCylinder('minimapMarker', { 
      height: 2, 
      diameter: 2 
    }, this.scene);
  ...
  private updateMinimapMarkers(): void {
    ...
    this.viewPoints.forEach(viewPoint => {
      ...
      // Crear marcador
      const marker = MeshBuilder.CreateCylinder(`minimap_marker_${viewPoint.id}`, {
        height: 1,
        diameter: 1
      }, this.scene);
      ...
      // Material según tipo
      const markerMaterial = new StandardMaterial(`minimap_marker_material_${viewPoint.id}`, this.scene);
      markerMaterial.diffuseColor = viewPoint.type === ViewType.EXTERIOR 
        ? new Color3(0.2, 0.6, 1.0) // Azul para exterior
        : new Color3(1.0, 0.6, 0.2); // Naranja para interior
  ...
  public getViewPointsByType(type: ViewType): ViewPoint[] {
    return Array.from(this.viewPoints.values())
      .filter(viewPoint => viewPoint.type === type);
  }
