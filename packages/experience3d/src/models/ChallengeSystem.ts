...
export class ChallengeSystem {
  ...
  constructor(
    ...
    this.options.showCompletedChallenges =
      this.options.showCompletedChallenges !== undefined
        ? this.options.showCompletedChallenges
        : true;
  ...
  private updateChallengeLists(): void {
    this.activeChallenges = Array.from(this.challenges.values()).filter(
      (challenge) => challenge.isActive && !challenge.isCompleted,
    );

    this.completedChallenges = Array.from(this.challenges.values()).filter(
  ...
  private generateSimulatedLeaderboard(): void {
    const usernames = [
      ...
      'PlanificadorExpert',
      'MagnateInmobiliario',
      'ArquitectoVisionario',
      'DesarrolladorCreativo',
    ];

    // Generar entradas simuladas
    this.leaderboard = Array(10)
      .fill(0)
      .map((_, index) => {
        const now = new Date();
