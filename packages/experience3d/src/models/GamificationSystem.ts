...
export class GamificationSystem {
    ...
    try {
      const data = {
        ...
        badgesProgress: Array.from(this.badges.entries())
          .filter(([_, badge]) => badge.progress !== undefined && badge.progress > 0)
  ...
  public getUnlockedBadges(): Badge[] {
    return Array.from(this.badges.values())
      .filter(badge => this.unlockedBadges.has(badge.id));
  }
  ...
  public getLevelProgress(): { current: number, required: number, percentage: number } {
    ...
    const nextLevelIndex = this.userLevels.findIndex(level => level.level === this.currentLevel.level + 1);
