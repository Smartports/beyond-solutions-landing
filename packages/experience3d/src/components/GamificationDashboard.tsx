...
import { GamificationSystem, Badge, UserLevel, BadgeCategory } from '../models/GamificationSystem';
...
const GamificationDashboard: React.FC<GamificationDashboardProps> = ({
  ...
  useEffect(() => {
    if (gamificationSystem) {
      ...
      gamificationSystem.onLevelUpCallback((newLevel, _oldLevel) => {
        setCurrentLevel(newLevel);
        setLevelProgress(gamificationSystem.getLevelProgress());
      });

      gamificationSystem.onBadgeUnlockedCallback((_badge) => {
        setUnlockedBadges(gamificationSystem.getUnlockedBadges());
      });
    }
  }, [gamificationSystem]);
  ...
  const getCategoryIcon = (category: BadgeCategory): string => {
    switch (category) {
      case BadgeCategory.DESIGN:
        return '🎨';
      case BadgeCategory.FINANCIAL:
        return '💰';
      case BadgeCategory.EXPLORATION:
        return '🔍';
      case BadgeCategory.SUSTAINABILITY:
        return '🌱';
      ...
      {showBadges && (
        <div className="badges-section">
          <div className="badges-header">
            <h3>
              Logros Desbloqueados ({unlockedBadges.length}/{allBadges.length})
            </h3>

            {showProgress && (
              <div className="badges-progress">
                <div className="progress-bar">
                  <div className="progress-fill" style={{ width: `${getBadgeProgress()}%` }} />
                </div>
          ...
          <div className={`badges-container layout-${badgeLayout}`}>
            ...
            {getFilteredBadges().length === 0 && (
              <div className="no-badges-message">No hay logros desbloqueados en esta categoría</div>
            )}

            {getFilteredBadges().length > maxBadgesToShow && (
              <div className="more-badges">+{getFilteredBadges().length - maxBadgesToShow} más</div>
            )}
          </div>
        </div>
      )}
